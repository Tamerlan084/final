...
public class PlayerDao implements InitializingBean {
    ...
    public Player update(Player player) {
        ...
        if (status != 0) {
            log.info("Player data updated for ID ");
        } else {
            player = null;
            log.warn("No Player found with ID " );
        }
        return player;
    }
    ...
    public List<Player> findFiveWithMaxPenalties() {
        List<Player> players = namedParameterJdbcTemplate.query(SQL_FIND_FIRST_FIVE_PLAYER_WITH_MAX_PENALTIES, new PlayerFiveMaxPenaltiesMapper());
        if (players != null) {
    ...
    public Player findById(int id) {
        ...
        if (player != null) {
            log.info("Successfully find player with ID " + player.getPlayerId());
        } else {
    ...
    private static final class PlayerFiveMaxPenaltiesMapper implements RowMapper<Player> {
